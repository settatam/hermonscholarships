/* MediaFit by Eric Winterstine v.11.01.2016
   --------------------------------------------------------------------------------
*/
var doMediaFit;
					...
					if ( pageMedia[i] && !pageMedia[i].loaded && !pageMedia[i].loading ) {
						...
						if ( !document.body.contains(pageMedia[i].domObject) ) {
							...
							if ( pageMedia[i].mediaType === 'video' ) {
								...
								mediaObject.loaded = false;
								mediaObject.mediaType = pageMedia[i].mediaType;
								mediaObject.domObject = pageMedia[i].domObject;
								mediaObject.position = pageMedia[i].mediaPosition;
								mediaObject.contain = pageMedia[i].mediaContain;
								mediaObject.flexContainer = pageMedia[i].flexContainer;
								mediaObject.failed = 0;
								mediaObject.indx = i;
								mediaObject.load();
								...
								mediaObject.onerror = function() {
									if ( this.failed > 10 ) {
										...
										// If we haven't pushed this yet to failed media, push it
										if ( this.failed === 0 ) {
											failedMedia.push(this);
										}
										setTimeout(function() {
											for ( var f = 0; f < failedMedia.length; f++ ) {
												if ( failedMedia[f].loaded === false ) {
													failedMedia[f].load();
												}
											}
										},500);
								...
								mediaObject.onloadeddata = function() {
									if ( this.indx !== null && document.body.contains(pageMedia[this.indx].domObject) ) {
										this.loaded = true;
										pageMedia[this.indx].loaded = true;
										pageMedia[this.indx].loading = false;
